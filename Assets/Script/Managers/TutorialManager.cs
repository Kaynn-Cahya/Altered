...
public class TutorialManager : Singleton<TutorialManager> {
    ...
    private void ShowDashAdvise() {
        LeanTween.scale(dashAdvise.gameObject, Vector3.one, 0.15f).setEaseSpring().setOnComplete(
    () => {
        timer = new Timer(10f, delegate {
            LeanTween.scale(dashAdvise.gameObject, Vector3.zero, 0.15f).setEaseLinear();
    ...
    private void TriggerGameOver() {
        ...
        dashAdvise.gameObject.SetActive(false);
    }
}
